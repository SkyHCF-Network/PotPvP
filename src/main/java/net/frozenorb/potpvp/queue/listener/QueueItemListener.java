    ...
    private Consumer<Player> joinSoloConsumer(boolean ranked) {
        return player -> {
            ...
            if (PotPvPValidation.canJoinQueue(player)) {
                if(ranked && PotPvPSI.getInstance().getMatchHandler().isRankedMatchesDisabled()){
                    player.sendMessage(ChatColor.RED + "Ranked matches are currently disabled.");
                    return;
                }
                if(!ranked && PotPvPSI.getInstance().getMatchHandler().isUnrankedMatchesDisabled()){
                    player.sendMessage(ChatColor.RED + "Unranked matches are currently disabled.");
                    return;
                }
        ...
        return kitType -> {
            ...
            return new CustomSelectKitTypeMenu.CustomKitTypeMeta(
                ...
                ranked ?  ImmutableList.of(
                        ChatColor.AQUA + "Fighting" + ChatColor.GRAY + ": " + ChatColor.WHITE + inFightsRanked,
                        ChatColor.AQUA + "Queueing" + ChatColor.GRAY + ": " + ChatColor.WHITE + inQueueRanked) :
                ImmutableList.of(
                    ChatColor.AQUA + "Fighting" + ChatColor.GRAY + ": " + ChatColor.WHITE + inFightsUnranked,
                    ChatColor.AQUA + "Queueing" + ChatColor.GRAY + ": " + ChatColor.WHITE + inQueueUnranked
                )
            );
        };
    }
