...
final class MatchScoreGetter implements BiConsumer<Player, LinkedList<String>> {
    ...
    public void accept(Player player, LinkedList<String> scores) {
        ...
        if (match == null) {
            if (followingOpt.isPresent()) {
                scores.add("&bFollowing&7: &r" + UUIDUtils.name(followingOpt.get()));
            }
        ...
        if (renderPing) {
            renderPingLines(scores, match, player, player);
        }
        ...
        // scores variable)
        if (followingOpt.isPresent()) {
            scores.add("&bFollowing: &7" + UUIDUtils.name(followingOpt.get()));
        }
    ...
    private boolean renderParticipantLines(List<String> scores, Match match, Player player) {
        ...
        if (ourTeamSize == 1 && otherTeamSize == 1) {
            render1v1MatchLines(scores, otherTeam, match, player);
    ...
    private void render1v1MatchLines(List<String> scores, MatchTeam otherTeam, Match match, Player player) {
        scores.add("&bFighting&7: &f" + FrozenUUIDCache.name(otherTeam.getFirstMember()));
       // renderMetaLines(scores, match, true);
            renderPingLines(scores, match, player, player);
    }
    ...
    private void renderJumboMatchLines(List<String> scores, MatchTeam ourTeam, MatchTeam otherTeam) {
        ...
        scores.add("&aTeam&7: &f" + ourTeam.getAliveMembers().size() + "/" + ourTeam.getAllMembers().size());
        scores.add("&cOpponents&7: &f" + otherTeam.getAliveMembers().size() + "/" + otherTeam.getAllMembers().size());
    }

    private void renderSpectatorLines(List<String> scores, Match match, MatchTeam oldTeam) {
        String rankedStr = match.isRanked() ? " (R)" : "";
        scores.add("&bKit&7: &f" + match.getKitType().getColoredDisplayName() + rankedStr);
        ...
        if (teams.size() == 2) {
            ...
            if (teamOne.getAllMembers().size() != 1 && teamTwo.getAllMembers().size() != 1) {
                ...
                if (oldTeam == null) {
                    scores.add("&aTeam One&7: &f" + teamOne.getAliveMembers().size() + "/" + teamOne.getAllMembers().size());
                    scores.add("&cTeam Two&7: &f" + teamTwo.getAliveMembers().size() + "/" + teamTwo.getAllMembers().size());
                } else {
                    MatchTeam otherTeam = oldTeam == teamOne ? teamTwo : teamOne;

                    scores.add("&aTeam&7: &f" + oldTeam.getAliveMembers().size() + "/" + oldTeam.getAllMembers().size());
                    scores.add("&bFighting&7: &f" + otherTeam.getAliveMembers().size() + "/" + otherTeam.getAllMembers().size());
                }
            }
        }
    }

    private void renderPingLines(List<String> scores, Match match, Player ourPlayer, Player player) {
        List<MatchTeam> teams = match.getTeams();
        if (teams.size() == 2) {
            ...
            if (firstTeamPlayers.size() == 1 && secondTeamPlayers.size() == 1 && !PotPvPSI.getInstance().getMatchHandler().isSpectatingMatch(player)) {
                scores.add("&7&b&4"); // spacer
                scores.add("&aYour Ping&7: &f" + PlayerUtils.getPing(ourPlayer));
                ...
                scores.add("&cTheir Ping&7: &f" + PlayerUtils.getPing(otherPlayer));
            }
        }
    }
    ...
    private void renderMetaLines(List<String> scores, Match match, boolean participant) {
        ...
        scores.add("&bTime&7: &f" + formattedDuration);
